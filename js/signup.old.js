		...
		$("#dialog-form").dialog({
			...
			buttons: {
				'Signup For KLECT': function(event) {
					...
					if (bValid) {
						$.tempThis = this;
						$.post(getBaseURL()+"index.php/login/create_member", $("#signup-form").serialize(),
								function (data)
								{
									if(data=="email")
									{
	...
	$("#forgotpw-div").dialog({
		...
		buttons: {
			'Email Password': function(event) {
				...
				if (bValid) {
					$.tempThis = this;
					$.post(getBaseURL()+"index.php/login/forgot_password", $("#forgotpw-form").serialize(),
							function (data)
							{
								if(data)
								{
